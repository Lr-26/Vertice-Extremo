...
const ExpeditionDetail = () => {
    ...
    return (
        <div className="bg-slate-950 min-h-screen text-white pb-32 selection:bg-brand/30 selection:text-brand-300">
            {/* Fixed Navigation/Back Button - Redesigned to be cleaner without the badge */}
            <div className="fixed top-0 left-0 w-full z-50 pt-20 px-10 md:px-20 flex justify-start items-center bg-gradient-to-b from-slate-950 to-transparent pointer-events-none">
                <button
                    onClick={() => navigate('/')}
            ...
            <div className="relative z-[30] bg-slate-950 shadow-[0_-150px_200px_rgba(2,6,23,1)]">
                <div className="container mx-auto px-6 pt-64 pb-48">
                    ...
                    <div className="md:col-span-8 space-y-16">
                        ...
                        {/* Moved Gallery - Under the video for Better Layout Flow */}
                            ...
                            <div className="grid grid-cols-2 lg:grid-cols-3 gap-6">
                                {expedition.gallery.map((img, i) => (
                                    <motion.div 
                                        ...
                                        className="h-64 rounded-3xl overflow-hidden border border-white/5 shadow-2xl relative group"
                                    >
                                        <img src={img} className="w-full h-full object-cover filter transition-all duration-700 brightness-75 group-hover:brightness-100" alt={`G-${i}`} />
                                        <div className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-slate-950 to-transparent opacity-0 group-hover:opacity-100 transition-opacity">
                                            <span className="text-[8px] font-bold text-white uppercase tracking-[0.3em]">Vista Detallada</span>
                                        </div>
                                    </motion.div>
                                ))}
                            </div>
                    ...
                    {/* Sidebar / Sticky Booking - Massive top margin to start very low and avoid overlaps */}
                    <div className="md:col-span-4 relative mt-20 md:mt-64">
                        <div className="md:sticky md:top-32 bg-slate-900 border border-white/5 p-12 rounded-[40px] shadow-[0_30px_60px_rgba(0,0,0,0.5)] z-10 transition-all overflow-hidden relative">
                            {/* Integrated Label Badge */}
                            <div className="absolute top-0 right-0 py-2 px-6 bg-brand text-slate-950 font-black uppercase tracking-[0.2em] text-[8px] rounded-bl-2xl shadow-xl">
                                Expedición Activa
                            </div>
